    ...
    for _ in range(Q):
        ...
        if op == 100:
            ...
            for i in range(3, len(tmp) - 2, 3):
                ...
                graph[v].append((w, u))
                graph[u].append((w, v))
            cache.clear()  # Clear cache as the graph has changed

        elif op == 200:
            id, revenue, dest = tmp[1], tmp[2], tmp[3]
            travel_cost = dijkstra(graph, start, dest, cache)
            benefit = revenue - travel_cost
            heapq.heappush(q, (-(benefit), id, dest, revenue))
            ...
            while q:
                benefit, id, dest, revenue = heapq.heappop(q)
                if poss[id] and benefit <= 0 and dijkstra(graph, start, dest, cache) != float('inf'):
                    print(id)
                    found = True
                    break
                temp_q.append((benefit, id, dest, revenue))
